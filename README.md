...
> **디버깅용 프리릴리스 · debug-2026.09.16**
> 현재 배포는 테스트용 소스 버전입니다. 시작 화면의 `DEBUG 관리자 로그인`에서 로컬 계정을 최초 설정한 뒤 전용 QA 세이브를 사용하세요. 공용 비밀번호는 제공하지 않습니다.
> 실행 방법, 테스트 범위와 알려진 문제는 [디버그 배포 안내](docs/debug-release-2026-09-16.md)를 참고하세요. EXE 배포가 아니므로 Python 설치가 필요합니다.
...
- 11월 FA 자격·승인·시장 개장에 따른 잔류 및 영입 기조 결정
- K-BASEBALL SERIES 대표팀 34명 소집, 구단별 차출·훈련·원정·복귀와 피로 반영
- 2차 드래프트 보호명단·지명 결과·합류 선수 역할 확정 흐름
- 11월 25일 보류선수 최종 제출, 27일 전력 결산과 30일 공시 반영
...
- 12월 이후 일정의 일부 업무는 화면과 규칙을 계속 확장하고 있습니다.
