...
> **현재 main: 디버깅용 버전 · debug-2026.09.16 기반**
> 안정판이 아닌 테스트용 소스 버전입니다. `main`에서도 디버그 로그인과 전용 QA 기능을 사용할 수 있습니다. EXE 배포가 아니므로 Python 설치가 필요합니다.

[디버그 릴리스](https://github.com/KBOFM2025/KBOFM2025/releases/tag/debug-2026.09.16) · [소스 ZIP 다운로드](https://github.com/KBOFM2025/KBOFM2025/releases/download/debug-2026.09.16/KBOFM2025-debug-2026.09.16-source.zip) · [배포 안내](docs/debug-release-2026-09-16.md) · [오류 제보](https://github.com/KBOFM2025/KBOFM2025/issues)

태그 ZIP은 고정된 배포본이며, `main`은 이후 수정이 반영될 수 있는 개발 브랜치입니다.
...
1. `run_game.bat`으로 실행한 뒤 시작 화면에서 **DEBUG 관리자 로그인**을 선택합니다.
2. 최초 실행 시 본인이 사용할 ID(4자 이상)와 비밀번호(문자·숫자 포함 8자 이상)를 등록합니다. 배포용 공용 비밀번호는 없습니다.
3. 로그인하여 전용 QA 세이브에 진입하고, 디버그 콘솔에서 일정·면담·FA·트레이드 등 이벤트를 점검합니다.
4. 연습경기는 디버그 세이브에서 라인업 설정 → 인트로 → 경기 진행 → 결과 순으로 테스트할 수 있습니다.

디버그 기능은 일반 세이브와 분리되어 있습니다. 로그인은 로컬 접근 확인 기능이며, 소스나 로컬 DB에 접근할 수 있는 사람을 차단하는 서버 인증은 아닙니다.

개인 세이브·인증 DB·로컬 수집 선수 사진·AI 모델은 배포에 포함하지 않습니다. 선수 사진이 없는 경우 기본 표시를 사용합니다.

### 기존 설치본 업데이트

게임을 완전히 종료하고 `data/kbo_fm_saves.db`와 `data/saves/`를 함께 백업한 뒤 업데이트하세요. 기존 개발판과 저장 형식이 다를 수 있으므로 이번 검증에는 새 QA 세이브를 권장합니다. 로컬에서 소스를 수정했다면 무조건 덮어쓰지 말고 변경사항을 먼저 보관하세요.
...
- 팀 주간 일정, 개인 훈련, 훈련 유닛, 코치 선임·담당 분야와 수석 코치 위임
- 선수별 10점 만점 훈련 평점, 최근 7일 평균·이전 주 대비 변화
- 휴식·재활·국가대표 차출일은 평점 평균에서 제외
- 나이·회복·훈련 중점·잠재 한계를 반영한 세부 능력 성장 및 하락 기록
- 프로 4년차까지 누적 성적에 따른 잠재력 재평가, 이후 자료 확인을 거쳐 고정

### FA 투수의 선발·불펜 전환

외국인 계약에서 현재 보직이 마무리여도 선발 보직을 제안할 수 있습니다. 국내 FA 투수는 `출장·등판 계획`에서 선발 기용을 제안합니다. 계약 후에는 라인업의 `선발 · 불펜` 화면에서 기존 배정을 비우고 새 보직에 배치한 뒤 저장하세요. 보직 변경만으로 체력이나 구종 능력이 자동 상승하지는 않습니다.
...
| `data/saves/` | 세이브별 선수 DB 등 개별 진행 데이터 |
...
세이브를 백업하려면 게임을 완전히 종료한 뒤 `data/kbo_fm_saves.db`와 `data/saves/`를 함께 복사하세요. 인증 정보가 포함될 수 있으므로 백업 DB를 공개 게시판에 올리지 마세요.
...
- 디버그 배포 직전 자동 테스트 122개가 통과했습니다. 모든 화면·이벤트의 완성이나 모든 PC에서의 실행을 보장하는 것은 아닙니다.
- 10구단·682명·30일 전원 위임 검증에서 363명이 성장했습니다. 성장 빈도·훈련 평점의 상향 편향·높은 회복량은 밸런스 조정 대상입니다.
- 연습경기에는 핵심 능력치가 반영되지만 모든 세부 능력이 연결되지는 않았고, 일부 인위적 공격 보정도 남아 있습니다.
- 해외 전체 경력이나 입단 정보가 미확인인 선수는 잠재력 확정을 보류합니다.
...
버그 제보에는 버전/커밋, Windows·Python 버전, 게임 날짜와 구단, 재현 순서, 기대 동작·실제 동작을 적어주세요. `data/logs/crash.log`를 첨부할 때는 사용자 경로 등 개인 정보를 가리고, 비밀번호나 인증 DB는 공유하지 마세요.
